...
import { getLocalizedDownloadCenter } from "@/lib/copy";
import { Download, Eye } from "lucide-react";

import { downloadCategoryIcons } from "@/lib/site-data";

export function DownloadCenter() {
  const { language } = useSiteLanguage();
  const { featured, items, labels } = getLocalizedDownloadCenter(language);

  return (
    <div className="space-y-6">
      <article className="glass-panel card-hover overflow-hidden rounded-card border border-gold/20 p-6 md:p-8">
        <div className="grid gap-8 lg:grid-cols-[0.92fr_1.08fr]">
          <div className={`relative aspect-[16/10] overflow-hidden rounded-[28px] bg-gradient-to-br ${featured.gradient}`}>
            ...
            <div className="absolute bottom-6 left-6 right-6 rounded-[24px] border border-white/10 bg-background/45 p-5 backdrop-blur-md">
              <p className="font-[var(--font-inter)] text-sm uppercase tracking-[0.24em] text-gold">{labels.featured}</p>
          ...
          <div className="flex flex-col justify-between">
            <div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-8 text-muted">{featured.description}</p>
            </div>
      ...
      <div className="grid gap-5 xl:grid-cols-3 md:grid-cols-2">
        {items.map((item, index) => {
          const iconKey = ["Whitepaper", "Mechanism", "Ecosystem", "Visual Assets"][index + 1] || "Mechanism";
          const Icon = downloadCategoryIcons[iconKey as keyof typeof downloadCategoryIcons];
          return (
